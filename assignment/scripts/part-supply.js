...
for (let i = 0; i < supplyChanges.length; i++) {
  console.log('inventory', supplyChanges);
  if(supplyChanges[i] > 0){positives.push(supplyChanges[i])}
  if(supplyChanges[i] < 0){negatives.push(supplyChanges[i])}
  if(supplyChanges[i] ===0){zeroes.push(supplyChanges[i])}
}
console.log(positives);console.log(negatives);console.log(zeroes);
//I'm feeling like there is probably a more elegant way to log multiple variables in the same code line.
